...
// Modifed Version of https://github.com/computerjazz/react-native-infinite-pager/blob/main/src/index.tsx
// Updated to have a default index and has misc ScrollView fixes

// Theirs also a good pager at https://github.com/bluesky-social/social-app/blob/82059b7ee138d24ff50b0f4fad0eaeac860bb78c/src/view/com/pager/Pager.tsx

import { pageInterpolatorSlide } from "./pageInterpolatorSlide";
...
interface Props {
  ...
  pageInterpolator?: typeof pageInterpolatorSlide;
...
interface ImperativeApiOptions {
  animated?: boolean;
}

export interface InfinitePagerImperativeApi {
...
function InfinitePager(
  ...
  const pageHeight = useSharedValue(height);
  const pageSize = vertical ? pageHeight : pageWidth;

  const translateX = useSharedValue(-defaultIndex * 375);
  const translateY = useSharedValue(0);
  const translate = vertical ? translateY : translateX;

  const minIndex = 0;
  const maxIndex = pageAmount;
  ...
  const pageInterpolatorRef = useRef(pageInterpolatorSlide);
  pageInterpolatorRef.current = pageInterpolatorSlide;
...
interface PageWrapperProps {
  ...
  isActive: boolean;
  style?: AnyStyle;
  pageInterpolatorRef: React.MutableRefObject<typeof pageInterpolatorSlide>;
  pageBuffer: number;
}

export interface PageInterpolatorParams {
  index: number;
  vertical: boolean;
  focusAnim: Animated.DerivedValue<number>;
